...
When a Plugin or App Module uses Core App Data as its Alpha data plane, its service API key must be bound to the runtime identity with `provider_plugin_id`, `plugin_id`, or `provider_id` metadata. Core then treats App Data models as owned by the installed manifest resource declaration: the service key can create, list, read, and mutate only models declared by that plugin and recorded with `owner_plugin_key` / `declared_resource_key`. Human sessions can still manage ordinary app-owned models, but cannot claim or edit plugin-owned models through reserved metadata fields.
