...
When a Plugin or App Module uses Core App Data as its Alpha data plane, its service API key must be bound to the runtime identity with the Core-owned `provider_runtime_plugin_id` API-key field. Core rejects provider runtime identity aliases in API-key metadata because metadata is caller-supplied extension data, not trusted runtime identity. The installed manifest must also declare a capability or local capability whose data plane allows `core_data_api`; a matching resource declaration alone is not enough.

Core treats App Data models as owned by the installed manifest resource declaration: the service key can create, list, read, and mutate only models declared by that plugin and recorded with first-class governance columns (`owner_plugin_key`, `declared_resource_key`, `owner_plugin_type`, `app_id`, `trust_bundle_id`, `owner_module_key`, `tenant_scoped`, and `audit_enforcement`). Those columns are mirrored into Resource Registry metadata for inspection, but metadata is not trusted as the source of truth. Human sessions can still manage ordinary app-owned models, but cannot claim or edit plugin-owned models through reserved metadata fields.

For Backend OS Alpha, Core App Data is the generic `core_data_api` path that can carry business records for a production small-to-medium app. The end-state provider schema path remains stronger: reusable Providers and mature App Modules should move high-volume or independently isolated data to Core-governed provider schemas with RLS, zero-DDL runtime roles, Core-run migrations, and contract-declared audit enforcement.
