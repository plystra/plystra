...
func openAPIRoutes() []openAPIRoute {
	return []openAPIRoute{
		...
		{Method: http.MethodPost, Path: "/api/v1/grants/revoke", Tag: "Capability Grants", ID: "revokeCapabilityGrants", Summary: "Revoke capability grants", Description: "Core revokes revocable mediated grants at any time by grant_id, member_id, parent_grant_id (the grant and its descendant lineage), or a stale provider binding_epoch. Because providers introspect before executing, revocation is effectively immediate.", Body: new(capabilityGrantRevokeRequest), Response: new(openAPIEnvelope[map[string]any]), Security: openAPIAdmin},
		{Method: http.MethodPost, Path: "/api/v1/grants/reconcile", Tag: "Capability Grants", ID: "reconcileCapabilityGrants", Summary: "Reconcile hanging capability grants", Description: "Scans a Space for mediated grants past their expected outcome window with no receipt and marks them missing (pending versus missing discipline; only missing is signal), then lapses expired active grants.", Body: new(capabilityGrantReconcileRequest), Response: new(openAPIEnvelope[map[string]any]), Security: openAPIAdmin},
