...
func openAPIRoutes() []openAPIRoute {
	return []openAPIRoute{
		...
		{Method: http.MethodPost, Path: "/api/v1/action-gateway", Tag: "Capability Grants", ID: "invokeControlledAction", Summary: "Invoke a controlled action through the Action Gateway", Description: "Brokered A3 path for controlled_action capabilities (payments, settlement, irreversible operations). Core owns the action entrypoint: it authorizes (principal AND caller AND scope AND call graph), records a separate authorization_decision and action_execution, signs and invokes the provider handler with an idempotency key, and classifies the outcome as succeeded, rejected, failed, or result_unknown (allow does not mean executed).", Body: new(actionGatewayRequest), Response: new(openAPIEnvelope[map[string]any]), Security: openAPIAdmin},
